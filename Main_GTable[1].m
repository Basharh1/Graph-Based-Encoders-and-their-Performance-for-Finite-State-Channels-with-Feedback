...
FSC.x       = 2;
FSC.y       = 2;
FSC.channel = 'Fading';  % 'BEC'/ 'BSC'/ 'ZS'(ising and Trapdoor)/ 'Pilot'/ 'Fading'/ 'DEC'/ 'Z_channel'
FSC.state   = 'S=X';    % 'DMC'/ 'S=X'/ 'S=Y'/ 'XOR(SXY)'/ 'XOR(SX)' / 'No_consecutive'/ 'Trapdoor_M'/ 'Fading_Type2', 
FSC.Ch_para = 0.2;      % Channel parameters: scalar or vector.

Graph.meth      = 'GP'; % 'GP'/ 'Markov'/ 'SingleGraph'
Graph.meth_para = 2;    % For 'GP' choose 1,...6. For 'Markov', up to 8.
Graph.LB_EXP    = 20;   % No. of experiments in the LB (initial random points). Default = 5.
tic
